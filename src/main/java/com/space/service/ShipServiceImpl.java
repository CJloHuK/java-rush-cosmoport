...
public class ShipServiceImpl implements ShipService {
    ...
    private void checkShipParams(Ship ship) {
        ...
        if (ship.getName() != null && (ship.getName().isEmpty() || ship.getName().length() > 50))
            throw new BadRequestException("Invalid Ship.name");

        if (ship.getPlanet() != null && (ship.getPlanet().isEmpty() || ship.getPlanet().length() > 50))
            throw new BadRequestException("Invalid Ship.planet");

        if (ship.getCrewSize() != null && (ship.getCrewSize() < 1 || ship.getCrewSize() > 9999))
            throw new BadRequestException("Invalid Ship.crewSize");

        if (ship.getSpeed() != null && (ship.getSpeed() < 0.01D || ship.getSpeed() > 0.99D))
            throw new BadRequestException("Invalid Ship.speed");

        if (ship.getProdDate() != null) {
            Calendar cal = Calendar.getInstance();
            cal.setTime(ship.getProdDate());
            if (cal.get(Calendar.YEAR) < 2800 || cal.get(Calendar.YEAR) > 3019)
                throw new BadRequestException("Invalid Ship.date");
        }
    }
